...
deviceTypes['sq-blaster']={
    ...
    onChange:function(){
        if(!deviceTypes['.sq-blaster'] || deviceTypes['.sq-blaster'].devices)
            $.getJSON('/api/device/category/.sq-blaster', function(devices){ deviceTypes['.sq-blaster'].devices=devices; $('#commands').empty(); deviceTypes['sq-blaster'].onChange(); });
    ...
    onServerSave:function(device, body, callback){
        ...
        xml2js.parseString(body.device, function(error, dev){
            ...
            $.each(dev.SQCommandInfo.command_info, function(index, item){
                device.commands[item.$.command_name]={type:'POST', contentType:'application/xml', data:'<docommand key="'+device.name+'" repeat= "0" seq="0" command="'+item.$.command_name+'" ir_data="'+item.$.ir_data+'" ch="0" />', url:'http://'+body.blaster+'/docmnd.xml', crossDomain:true, headers:{'Connection':'close'}};
            });
            device.subDevices=[{name:'power', commands:{on:function(){ return item['POWER ON']}, off:function(){ return item['POWER OFF']; }, toggle:function(){ return item['POWER TOGGLE']}}}];
        });
    }
}; 